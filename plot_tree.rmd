...
Set your working directory, which has the tree file in it (and other files of interest, e.g. taxon sets)
...
```{r}
setwd("~/WORKING/GAP_Conservation/Isopogon/full/svd_Isopogon/phylo_ind/")
treefile <- "concord.tre"
...
```{r}
svg("test_out.svg", width = 10, height = 30)
```

(pdf otherwise)
```{r}
pdf("test_out.pdf", width = 10, height = 30)
```
...
Collapse clades into triangles with/without an annotation
Perhaps modify N. Cusimano's function (https://www.en.sysbot.bio.lmu.de/people/employees/cusimano/use_r/mapping.R)?
See: https://stackoverflow.com/a/34405198
We can use earlier plots to decide which clades we want to collapse

First, determine the formatting for the plot and plot it (set up coordinates).
Change this function to the way you want the tree to look.
```{r}
...
edge_combos <- tree$edge[all_edges, ]	# the edges are specific to the unaltered tree
...
```{r}
concord_df <- data.frame(matrix(ncol = 4, nrow = tree$Nnode))
index <- 1
...
concord_df[, 4] <- abs(100 - (concord_df[, 1] + concord_df[, 2] + concord_df[, 3]))
```

Plot concordance as a pie chart on nodes
```{r}
nodelabels(pie = concord_df,
			piecol = c("white", "lightgrey", "darkgrey", "black"),
			cex = 0.5)
```
