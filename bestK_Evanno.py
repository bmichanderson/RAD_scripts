...
# Author: B.M. Anderson
# Date: Oct 2021
# Modified: Aug 2022 (changed font and output file format); Feb 2026 (cleaned up and removed warnings)
...
parser.add_argument('-l', type = str, dest = 'like_file', help = 'The likelihoods file for the K reps (multiple lines per K)' +
	'in tab delimited form (first two columns): <K_value>	<likelihood>')
...
like_df.rename(columns = {	0: 'K', 1: 'Likelihood'}, inplace = True)
...
like_comps = like_df.groupby('K').agg(['mean', 'std'])
...
like_comps.plot(y = 'deltaK', marker = '.', markersize = 12,
	title = '\u0394K', legend = False, color = 'steelblue', ax = ax2)
