...
```{r}
install.packages(c("ape", "devtools", "directlabels", "leaflet", "leaflet.minicharts", "plotly", "remotes"))
...
mygenl <- gl.read.dart(snps_file, ind.metafile = meta_file,
	mono.rm = FALSE, recalc = TRUE, probar = TRUE)
```
...
mygenl <- gl.drop.ind(mygenl, clones, mono.rm = FALSE, recalc = TRUE)
...
# Remove monomorphic and secondary SNPs
...
writeLines(as.character(ncol(distance)), con = outfile)
...
```
OR  
Subsample to the same number per population
```{r}
pop_filt_genl <- gl.subsample.ind(pop_filt_genl, 4, by.pop = TRUE)
```
