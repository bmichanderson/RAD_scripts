...
Optionally drop outgroups (if not done via `meta.csv`)
```{r}
...
Check the unfiltered data (and plot to file, if desired, by running the bounding statements below)
```{r}
pdf("unfiltered.pdf")
```
Read depth
...
# Run popgen stats estimates
First, drop individuals if necessary
```{r}
drop_inds <- c(1017,
1018,
1034,
...
1043,
1044)
pop_genl <- gl.drop.ind(filtered_genl, drop_inds, recalc = TRUE,
	mono.rm = TRUE)
```
...
Drop pops with low coverage  
```{r}
drop_pops <- c("R04_ere",
"R23_ere",
"R24_ere")
pop_genl <- gl.drop.pop(pop_genl, drop_pops, recalc = TRUE,
	mono.rm = TRUE)
```

Estimate basic stats and write to file
```{r}
stde <- function(x) {
...
Estimate Reich et al. 2009 Fst (accounting for differing sample sizes) and write to file
```{r}
pops <- unique(pop_genl$pop)
...
cat("\n")
### export the pairwise Fst values to file
write.table(fsts_reich, file = "popgen_Reich_Fst.txt",
	quote = FALSE, row.names = TRUE)
```

Run IBD if interested (will use STaMPP Fst)
```{r}
pdf("IBD.pdf", width = 7, height = 7)
gl.ibd(pop_genl)
invisible(dev.off())
```

Alternatively, subset to the populations of interest for IBD  
(then rerun the above with a different output name)
```{r}
drop_pops <- c("R09_Ham",
"R08_Ham",
"R07_Ham",
...
"R12_Ham",
"R11_Ham")
pop_genl_ere <- gl.drop.pop(pop_genl, drop_pops, recalc = TRUE,
	mono.rm = TRUE)
```
...
"R08_ere",
"R06_ere")
pop_genl_Ham <- gl.drop.pop(pop_genl, drop_pops, recalc = TRUE,
	mono.rm = TRUE)
```
...
"R11_Ham",
"R03_ere")
pop_genl_erenoR3 <- gl.drop.pop(pop_genl, drop_pops, recalc = TRUE,
	mono.rm = TRUE)
```
...
"R11_Ham",
"R12_Ham")
pop_genl_HamnoR11R12 <- gl.drop.pop(pop_genl, drop_pops, recalc = TRUE,
	mono.rm = TRUE)
```
